		...
		if ((!(this->paused))){
			...
			if ((!(this->special))){
				...
				HX_STACK_LINE(117)
				{
					...
					Array< ::motion::actuators::PropertyDetails > _g1 = this->propertyDetails;		HX_STACK_VAR(_g1,"_g1");
					HX_STACK_LINE(117)
					while(((_g < _g1->length))){
						HX_STACK_LINE(117)
						::motion::actuators::PropertyDetails details1 = _g1->__get(_g);		HX_STACK_VAR(details1,"details1");
						HX_STACK_LINE(117)
						++(_g);
						HX_STACK_LINE(119)
						if ((details1->isField)){
...
{
	Static(__mClass) = hx::RegisterClass(HX_CSTRING("motion.actuators.MotionPathActuator"), hx::TCanCast< MotionPathActuator_obj> ,sStaticFields,sMemberFields,
